    ...
    partial class HFDownloader
    {
            ...
            bool resumeDownload = false, string? endpoint = null, IProgress<int>? progress = null)
        {
            ...
            if (File.Exists(blobPath) && !forceDownload)
            {
                ...
                {
                    // no symlink support:
                    throw new FileNotFoundException($"Path: {blobPath} not found");
                }
            }
            ...
            // TODO: add lock here. Remove it now because of BUG.
            {
                ...
                {
                    await HttpDownloadWithStreamAsync(client, uriToDownload, fs, proxy, resumeSize, headers,
                        expectedSize is null ? -1 : expectedSize.Value, progress: progress);
                }
                ...
                else
                {
                    ...
                    if ((localDirUseSymlinks is not null && localDirUseSymlinks.Value) ||
                        (localDirUseSymlinks is null && isBigFile))
                    {
        ...
        static async Task HttpDownloadWithStreamAsync(HttpClient client, Uri uri, FileStream fs,
            string? proxy = null, long resumeSize = 0,
            IDictionary<string, string>? headers = null, long expectedSize = -1, int retryLeft = 5,
            IProgress<int>? progress = null)
        {
            using (var request = new HttpRequestMessage(HttpMethod.Get, uri))
            {
                ...
                using (var response = await Utils.HttpRequestWrapperAsync(client, request, false, true))
                {
                    ...
                    response.Content.Headers.TryGetValues("Content-Length", out var lengths);
                    var lengthsArray = lengths as string[] ?? lengths.ToArray();
                    Debug.Assert(lengths != null && lengthsArray.Length > 0);
                    // NOTE: 'total' is the total number of bytes to download, not the number of bytes in the file.
                    // If the file is compressed, the number of bytes in the saved file will be higher than 'total'.
                    if (long.TryParse(lengthsArray?.First(), out var totalFileLengths))
                    {
                    ...
                    using (var responseStream = await response.Content.ReadAsStreamAsync())
                    {
                        ...
                        catch (IOException ex)
                        {
                            ...
                            {
                                Logger?.LogWarning(
                                    $"Error while downloading from {uri.AbsoluteUri}: {ex.Message}\nMax retries exceeded.");
                            ...
                            await HttpDownloadWithStreamAsync(GetClientForFileDownload(proxy), uri, fs, proxy,
                                newResumeSize, headers, expectedSize, retryLeft - 1, progress);
                            return;
                        }
                    }
        ...
        static HttpClient GetClientForFileDownload(string? proxy = null)
        {
            // download the file
            ...
            int timeout = -1, IDictionary<string, string>? userAgent = null)
        {
            ...
            client.Timeout = timeout == -1
                ? TimeSpan.FromSeconds(HFGlobalConfig.DefaultEtagTimeout)
        ...
        static void CacheCommitHashForSpecificVersion(string storageFolder, string revision, string commitHash)
        {
        ...
        static void CheckDiskSpace(long expectedSize, string targetDir)
        {
            var drive = new DriveInfo(Directory.GetDirectoryRoot(targetDir));
            if (drive.AvailableFreeSpace < expectedSize)
            {
                Logger?.LogWarning(
                    $"Not enough disk space to download the file: {expectedSize} bytes are needed, but only {drive.AvailableFreeSpace} bytes are available.");
            }
        }

        static string GetPointerPath(string storageFolder, string revision, string relativeFilename)
        {
        ...
        static void ChmodAndReplace(string src, string dst)
        {
            Logger?.LogWarning(
                "Access control is ignored in current version when copying from temp file to target folder.");
            File.Move(src, dst);
        }

        static string ToLocalDir(string path, string localDir, string relativeFilename, bool? useSymlinks)
        {
            var localDirFilepath = Path.Combine(localDir, relativeFilename);
