...
namespace HuggingfaceHub
{
    partial class HFDownloader
    {
            ...
            IGroupedProgress? progress = null, string? endpoint = null)
        {
            ...
            var filteredRepoFiles = Utils.FilterRepoObjects(repoInfo!.Siblings.Select(f => f.Filename), allowPatterns,
                ignorePatterns);
            ...
            if (totalTasks > 0)
            {
                foreach (var repoFilename in filteredRepoFiles)
                {
                    ...
                    var res = await DownloadFileAsync(repoId, repoFilename, revision: revision, cacheDir: cacheDir,
                        localDir: localDir,
        ...
        {
            string _filename;
            IGroupedProgress _groupedProgress;
