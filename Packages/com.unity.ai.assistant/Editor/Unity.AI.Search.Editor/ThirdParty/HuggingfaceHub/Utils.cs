...
namespace HuggingfaceHub
{
    static class Utils
    {
        ...
        /// <returns></returns>
        public static IEnumerable<string> FilterRepoObjects(IEnumerable<string> items, string[]? allowPatterns,
            string[]? ignorePatterns)
        {
            foreach (var item in items)
            {
                // Skip if there's an allowlist and path doesn't match any
                if (allowPatterns is not null && allowPatterns.Length > 0 &&
                    allowPatterns.All(p => !FileNameMatches(item, p)))
                {
                    continue;
                }

                // Skip if there's a denylist and path matches any
                if (ignorePatterns is not null && ignorePatterns.Length > 0 &&
                    ignorePatterns.Any(p => FileNameMatches(item, p)))
                {
                    continue;
                }
        ...
        /// <returns></returns>
        public static async Task<HttpResponseMessage> HttpRequestWrapperAsync(HttpClient client,
