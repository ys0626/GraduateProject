...
namespace HuggingfaceHub
{
    partial class HFDownloader
    {
        ...
        /// <returns></returns>
        public static async Task<ModelInfo?> GetModelInfoAsync(string repoId, string revision = "main",
            int? timeout = null,
            bool securityStatus = false, bool filesMetadata = false, string? token = null, string? endpoint = null)
        {
            ...
            string path = revision.Equals("main")
            ...
            NameValueCollection queryString = Utilities.HttpUtility.ParseQueryString(string.Empty);
            if (securityStatus)
            {
