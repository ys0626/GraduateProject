...
{
    class TextureGeneratorWindow : EditorWindow, IAssetEditorWindow
    {
        public static void Display(string assetPath)
        {
            var asset = new AssetReference { guid = AssetDatabase.AssetPathToGUID(assetPath) };
            var window = EditorWindowExtensions.CreateAssetWindow<TextureGeneratorWindow>(asset, $"Generate {Path.GetFileNameWithoutExtension(assetPath)}");
        ...
        {
            this.EnsureContext();
            rootVisualElement.Add(new TextureGenerator());
        }
