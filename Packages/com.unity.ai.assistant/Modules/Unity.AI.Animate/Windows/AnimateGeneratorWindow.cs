...
{
    class AnimateGeneratorWindow : EditorWindow, IAssetEditorWindow
    {
        public static void Display(string assetPath)
        {
            var asset = new AssetReference { guid = AssetDatabase.AssetPathToGUID(assetPath) };
            var window = EditorWindowExtensions.CreateAssetWindow<AnimateGeneratorWindow>(asset, $"Generate {Path.GetFileNameWithoutExtension(assetPath)}");
        ...
        {
            this.EnsureContext();
            rootVisualElement.Add(new AnimateGenerator());
        }
