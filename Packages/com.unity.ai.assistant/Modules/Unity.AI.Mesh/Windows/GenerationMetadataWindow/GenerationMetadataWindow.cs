...
namespace Unity.AI.Mesh.Windows
{
    class GenerationMetadataWindow : EditorWindow
    {
        ...
        public static async Task Open(IStore store, AssetReference asset, VisualElement element, MeshResult meshResult)
        {
            ...
            if (meshResult != null)
            {
                 window.generationMetadata = await meshResult.GetMetadata();
                 window.RefreshView();
            }
