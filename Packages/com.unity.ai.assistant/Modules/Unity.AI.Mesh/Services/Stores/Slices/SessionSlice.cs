...
namespace Unity.AI.Mesh.Services.Stores.Slices
{
    static class SessionSlice
    {
        ...
        {
            var settings = MeshGeneratorSettings.instance.session;
            ...
            store.CreateSlice(
                ...
                extraReducers => extraReducers
                    .AddCase(AppActions.init).With((_, payload) =>
                    {
                        ...
                        foreach (var kvp in MeshGeneratorSettings.instance.session.settings.lastSelectedModels)
                        {
                        ...
                        mergedState.settings.previewSettings.sizeFactor = MeshGeneratorSettings.instance.session.settings.previewSettings.sizeFactor;
                        return mergedState;
                    })
