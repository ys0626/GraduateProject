...
namespace Unity.AI.Mesh.Services.Utilities
{
    ...
    {
        public static void BindTo<T>(this GridView gridView, List<GenerationTile> itemPool, Func<bool> replaceAssetOnSelect = null) where T: MeshResult
        {
            ...
            gridView.bindItem = (element, i) =>
            {
                ...
                if (itemPool.Count > 0)
                {
                    var found = itemPool.FindIndex(item => item.meshResult == result);
                    if (found < 0)
            ...
            gridView.selectedIndicesChanged += async indexes =>
            {
                ...
                if (gridView.GetAsset() == null || values.Count <= 0 || textures.Count <= values[0] || textures[values[0]] == null ||
                    textures[values[0]] is MeshSkeleton)
                    return;
