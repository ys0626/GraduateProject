...
namespace Unity.AI.Pbr.Components
{
    ...
    partial class GenerationSelector : VisualElement
    {
        ...
        const string k_Uxml = "Packages/com.unity.ai.assistant/modules/Unity.AI.Pbr/Components/GenerationSelector/GenerationSelector.uxml";
        ...
        {
            m_HasActiveSkeletons = materials.Any(result => result is MaterialSkeleton);
            m_GenerationFileSystemWatcher?.SetActivePolling(m_HasActiveSkeletons);
            ...
            if (assetMonitor)
            {
                ...
                // while we're inside OnGeneratedMaterialsChanged, the selector may not re-fire.
                if (!m_PruneScheduled)
                {
                ...
                (files, isInitialLoad) =>
                {
                    if (this.SelectWindowSettingsDisablePrecaching())
                        this.Dispatch(GenerationResultsActions.setGeneratedMaterials,
                            new(asset, files.Select(MaterialResult.FromPath).ToList(), isInitialLoad));
                    else
