...
namespace Unity.AI.Sound.Windows
{
    class GenerationMetadataContent : VisualElement
    {
        ...
        const string k_Uxml = "Packages/com.unity.ai.assistant/modules/Unity.AI.Sound/Windows/GenerationMetadataWindow/GenerationMetadataWindow.uxml";
        ...
        void InitVoice()
        {
            var voiceContainer = this.Q<VisualElement>(className: "voice-container");
            var voiceMetadata = this.Q<Label>("voice-metadata");
            var voiceCopyButton = this.Q<Button>("copy-voice-button");
            var voiceUseButton = this.Q<Button>("use-voice-button");

            var voice = m_GenerationMetadata?.voice;
            voiceMetadata.text = voice;
            voiceCopyButton.clicked += () =>
            {
                EditorGUIUtility.systemCopyBuffer = voice;
            };
            voiceUseButton.clicked += UseVoice;
            voiceContainer.EnableInClassList("hidden", string.IsNullOrEmpty(voice));
        }
        ...
        void UseVoice()
        {
            if (!string.IsNullOrEmpty(m_GenerationMetadata?.voice))
                this.Dispatch(GenerationSettingsActions.setDynamicParam, new KeyValuePair<string, string>("voice", m_GenerationMetadata.voice));
        }
